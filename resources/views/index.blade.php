  ...
  <div id="about" class="container mt-4 px-2 px-lg-3">
    <div class="card">
      ...
      <div class="card-body p-sm-4">
        ...
        <p>
          去る 2020 年 12 月、ニコニコ実況は<a href="https://blog.nicovideo.jp/niconews/143148.html" target="_blank">ニコニコ生放送内の一公式チャンネルとしてリニューアルされました。</a><br>これに伴い、2009 年 11 月から運用されてきた旧システムは提供終了となり（事実上のサービス終了）、torne や BRAVIA などの家電への対応が軒並み終了する中、当時の生の声が詰まった約 11 年分の過去ログも同時に失われることとなってしまいました。
        </p>
        <p>
          そこで 5ch の DTV 板の住民が中心となり、旧ニコニコ実況が終了するまでに 11 年分の全チャンネルの過去ログをアーカイブする計画が立ち上がりました。紆余曲折あり Nekopanda 氏が約 11 年分のラジオや BS も含めた全チャンネルの過去ログを完璧に取得してくださったおかげで、11 年分の過去ログが電子の海に消えていく事態は回避できました。<br>
        ...
        <p class="mb-0">
          この API は、そうしたニコニコ実況の過去ログを取り巻く現状を改善すべく、Nekopanda 氏が配布されている旧ニコニコ実況の 2020/12/15 までの全過去ログに加え、15 分に 1 回コミュニティからの番組も含めた新ニコニコ実況の当日分の過去ログを自動で収集し、取得したデータを XML 形式や JSON 形式で提供する、非公式過去ログデータベース API です。</span><br>
        ...
        <ul class="mb-0">
          ...
          <ul>
            ...
            <ul>
              ...
              <li>17:00 ～ 17:15 の過去ログの収集が終わる 17:15 以降（実際は実行に 3 分ほどかかるため 17:18 以降）まで待つ必要があります。</li>
            ...
            <ul>
              ...
              <li>このため、API を利用するクライアントソフト側で運営コメントを全て弾いたり、<code>/nicoad</code> からメッセージだけ取り出して固定コメントとして描画したり…といった実装が別途必要になります。</li>
              <li>正規表現なら <code>/\/[a-z]+ /</code> で判定できると思います。</li>
            </ul>
          </ul>
          <li><span class="text-danger">指定された期間の過去ログが存在しない場合は空の packet が返されます。</span></li>
          <ul>
            <li>例えば 2009 年 11 月 26 日（ニコニコ実況のリリース日）よりも前の時刻や、現在時刻よりも未来の時刻などです。</li>
          ...
          <li><span class="text-info">サーバーは 3 時と 15 時に再起動します。その関係で、再起動後 2 ～ 3 分は API にアクセスできなくなります。</span></li>
      ...
      <p>
        ...
        <table class="table table-response, table-response-error">
          ...
          <tr>
            ...
            <td>
              リクエストパラメータのうちのいずれかが欠けている場合に発生します。<br>
          ...
          <tr>
            ...
            <td>
              取得開始時刻・取得終了時刻の UNIX タイムスタンプがマイナスの値や現在時刻より未来の値になっている場合に発生します。<br>
            </td>
          </tr>
          <tr>
            <th>取得開始時刻と取得終了時刻が同じ時刻です。</th>
            <td>
              取得開始時刻と取得終了時刻の UNIX タイムスタンプが同じ値の場合に発生します。<br>
            </td>
          </tr>
          <tr>
            <th>指定された取得開始時刻は取得終了時刻よりも後です。</th>
            <td>
              取得開始時刻の UNIX タイムスタンプが取得終了時刻の UNIX タイムスタンプよりも大きい場合に発生します。<br>
            </td>
          </tr>
          <tr>
            <th>3日分を超えるコメントを一度に取得することはできません。数日分かに分けて取得するようにしてください。</th>
            <td>
              サーバーの処理負荷等の兼ね合いにより、3日分を超えるコメントを一度に取得することはできません。数日分かに分けて取得するようにしてください。<br>
            </td>
          </tr>
        </table>
